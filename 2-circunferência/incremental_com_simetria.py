...
def gerar_circunferencia(xc, yc, r, matriz, num_linhas, num_colunas):
    ...
    y = 0
    teta = 1/r 
    C = cos(teta) #são sempre valores fixos
    S = sin(teta)
    while y <= x + 1:
        liga_pixel(arredondar(x + xc), arredondar(y + yc), matriz, num_linhas, num_colunas) #usa a simetria
        liga_pixel(arredondar(y + xc), arredondar(x + yc), matriz, num_linhas, num_colunas) 
        liga_pixel(arredondar(-y + xc), arredondar(x + yc), matriz, num_linhas, num_colunas)
        liga_pixel(arredondar(-x + xc), arredondar(y + yc), matriz, num_linhas, num_colunas)
        liga_pixel(arredondar(-x + xc), arredondar(-y + yc), matriz, num_linhas, num_colunas)
        liga_pixel(arredondar(-y + xc), arredondar(-x + yc), matriz, num_linhas, num_colunas)
        liga_pixel(arredondar(y + xc), arredondar(-x + yc), matriz, num_linhas, num_colunas)
        liga_pixel(arredondar(x + xc), arredondar(-y + yc), matriz, num_linhas, num_colunas)
        #tive que arrendonar por conta do uso de números reais
        xt = x
        x = x*C-y*S
        y = y*C+xt*S
